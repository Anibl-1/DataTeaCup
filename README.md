...
| **AI 模型配置（10+ 多模型接入）** | **AI 对话式开发** |
| ![AI 模型配置](docs/assets/screenshots/datateacup-ai-config-preview.png) | ![AI 对话式开发](docs/assets/screenshots/datateacup-ai-chat-dev-preview.png) |
| **页面设计器（低代码大屏）** | **系统监控** |
| ![页面设计器](docs/assets/screenshots/datateacup-page-designer-preview.png) | ![系统监控](docs/assets/screenshots/datateacup-system-monitor-preview.png) |
...
内置 **AI 数据助手**，不绑定厂商，覆盖从「选模型 → 对话取数 → 一键生成报表/图表/ETL → 数据洞察」的全链路，让自然语言成为操作平台的入口。

#### 多模型接入（10+ 厂商，开箱即用）

可视化「AI 模型配置」面板，无需改代码即可切换模型，每个厂商内置主流模型选项并可自定义 API 地址：

| 厂商 | 内置模型（节选） |
| --- | --- |
| **OpenAI** | GPT-4.1 / 4.1 Mini / 4o / o3 / o4-mini / GPT-4 Turbo |
| **DeepSeek** | DeepSeek V4 Flash / V4 Pro |
| **通义千问 Qwen** | Qwen-Max / Plus / Turbo / Long / Qwen2.5-Coder |
| **Claude** | Claude Sonnet 4 / 3.5 Sonnet / 3.5 Haiku / 3 Opus |
| **Gemini** | Gemini 2.5 Flash / 2.5 Pro / 2.0 Flash / 1.5 Pro |
| **智谱 GLM** | GLM-4-Plus 等 |
| **Ollama（本地）** | 本地私有化部署模型 |
| **DeepSeek（本地）** | DeepSeek-R1 / Coder-V2 等本地推理 |
| **百度文心、讯飞星火** | 国产大模型接入 |

- **灵活配置**：自定义 API 密钥 / API 地址、选择具体模型、调节 **记忆轮数**（AI 记住最近几轮对话）、一键 **测试连接**，支持「仅保存 / 保存配置」。
- **数据安全**：可选 **Ollama / DeepSeek 本地模型**，数据不出内网，满足私有化与合规要求。

#### 对话式开发（自然语言直接产出资产）

AI 助手首页提供快捷指令卡片 + 推荐问题，选择数据源后即可对话：

- **创建**：💬 创建报表、创建图表、创建 ETL、生成 SQL —— 一句话生成并直接落库（含自动建菜单）。
- **分析**：质量分析、数据字典解读、智能建议（索引优化 / 慢查询分析等）。
- **查看**：报表列表、ETL 任务、系统诊断与系统总览。
- **附件上传**：支持上传文件（Excel / CSV / 文档等），由 AI 解析结构与内容并辅助分析（`/ai/analyze-file`）。
- **流式对话**：SSE 实时流式输出（`/ai/chat/stream`），多会话管理、历史保存与检索。

#### 智能能力清单

- **智能 SQL 生成**：自动加载表结构，自然语言 → 可执行 SQL（`/ai/generate-sql`）。
- **SQL 优化与解释**：性能优化建议与逐句解释（`/ai/optimize-sql`、`/ai/explain-sql`）。
- **数据分析与洞察**：结果 AI 解读、异常检测、趋势分析、智能告警与洞察报告（`AiInsightController`）。
- **AI 图表 / 报表 / ETL / 大屏**：自然语言一键生成并落库（`/ai/generate-and-create-chart`、`/ai/create-report-with-menu`、`/ai/generate-etl`、`NLDashboardController`）。
- **AI 血缘分析**：基于 SQL / 元数据自动推断字段级血缘（`/ai/analyze-lineage`）。
- **Prompt 模板**：内置 + 自定义模板库，沉淀团队最佳实践（`/ai/prompt-templates`）。
- **用量统计与额度**：调用次数统计与每日额度控制（开源版默认每日 20 次）。

### 🎨 页面设计器与可视化大屏（低代码）

拖拽式低代码搭建引擎，零代码即可搭出数据门户页面、业务表单与可视化大屏。

- **画布与布局**：网格画布、画布尺寸预设（如 1200×800 经典）、自由缩放、显示参数区、全屏设计、主题切换（默认 / 暗黑等）。
- **丰富组件库**（拖拽即用）：
  - **基础组件**：文本框、数字框、日期、日期树、下拉单选、下拉多选、滑块选择、按钮、图片、容器、分割线、表单、超链接等；
  - **基础图表**：折线图、柱状图、饼图 / 环形图、数据表格、汇总表、进度表、雷达图等（与图表中心同源）；
  - **查询组件**：参数筛选、查询面板，配合数据集实现交互式数据应用。
- **数据绑定与参数**：组件绑定数据源 / 图表 / 报表，支持页面级动态参数与组件联动（`DynamicParameterController`、参数面板）。
- **图层 / 模板 / AI 助手**：图层管理、模板库快速起步、内置 AI 助手按描述生成页面布局。
- **版本与发布**：版本历史回溯、在线预览、独立页面/大屏地址（`/bigscreen-view/:id` 全屏大屏、`page-view/:id` 页面）、移动端访问与图表嵌入（`/embed/chart/:id`）。
